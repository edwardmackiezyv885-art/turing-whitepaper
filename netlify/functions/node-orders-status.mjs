...
export default async function handler(request) {
  ...
  try {
    ...
    if (body.recoverPayment === true && !PAID_STATUSES.has(order.status)) {
      try {
        ...
        // Order status remains available even if the RPC recovery check is temporarily unavailable.
      }
    }
    return json(200, { order: publicOrder(order), paymentCandidates, recoveryBlockHeight });
  } catch (error) {
